...
/* Glifo oficial do WhatsApp: bolha + fone interno (path único, mesma
   silhueta usada na bolinha flutuante — agora compartilhado entre
   todos os CTAs pra padronizar a leitura visual). */
const WA_GLYPH_PATH =
...
export function WhatsAppFloating({ message }: { message?: string }) {
  /* Visível só a partir da segunda sessão (About). Enquanto o Hero
     ainda toca o viewport, escondemos. IntersectionObserver com
     threshold 0 cobre o estado "qualquer pixel do Hero visível". */
  ...
  return (
    <a
      ...
      rel="noopener noreferrer"
      onClick={handleClick}
      className={cn("wa-float", visible && "is-visible")}
      aria-hidden={!visible}
